...
def train(model, train_loader, validate_loader):
    ...
    validat_loss_acc = []
    out_file = open('./Result/VGGB_S_on_CIFAR10.txt', 'a')
    ...
    for epoch in range(num_epochs):
        ...
        for i, (images, labels) in enumerate(train_loader):
            ...
            if iter % 500 == 0:
                ...
                # print(model.features[0].old_filters.data)

        if failure >= 3:
            break

    out_file.close()
    np.save('./Result/VGGB_S_on_CIFAR10.npy', np.array(validat_loss_acc))
...
def get_model():
    model = vgg13_bn(pretrained=False, num_classes = 10)
    # use an easy trick
